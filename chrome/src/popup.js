...
class PopupController {
  ...
  async loadSettings() {
    const result = await chrome.storage.sync.get(['enabled', 'enableNewSites', 'enabledSites', 'disabledSites', 'customParams', 'infiniteScroll']);
  ...
  async toggleSiteEnabled() {
    ...
    const result = await chrome.storage.sync.get(['enabledSites', 'disabledSites']);
    ...
    await chrome.storage.sync.set({ enabledSites, disabledSites });
    await this.loadSettings();
    this.updateStatus();
  ...
  async setEnableNewSites(enabled) {
    await chrome.storage.sync.set({ enableNewSites: enabled });
  ...
  async setInfiniteScroll(enabled) {
    await chrome.storage.sync.set({ infiniteScroll: enabled });
  ...
  async retryDetection() {
    ...
    try {
      await chrome.tabs.sendMessage(this.currentTab.id, { type: 'RETRY_DETECTION' });
    } catch (error) {
